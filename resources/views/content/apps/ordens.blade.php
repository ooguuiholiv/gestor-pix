...
@section('page-script')
<script>
    var loadDataUrl = '{{ route('ordens.list') }}';
    var destroyMultipleUrl = '{{ route('ordens.destroy_multiple') }}';
    var label_update = '{{ __('messages.update') }}';
    var label_delete = '{{ __('messages.delete') }}';
</script>
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script src="{{ asset('assets/js/pages/ordens.js') }}"></script>
<script>
...
@section('content')
<h4 class="py-3 mb-2">
  <span class="text-muted fw-light">{{ config('variables.templateName', 'TemplateName') }} / </span> Lista de Pedidos
</h4>
...
<div class="card">
  <div class="card-body">
    ...
    <table id="table" data-toggle="table" data-loading-template="loadingTemplate"
        data-url="{{ route('ordens.list') }}" data-icons-prefix="bx" data-icons="icons"
      ...
      <thead>
        <tr>
          ...
          <th data-field="id" data-visible="{{ in_array('id', $visibleColumns) || empty($visibleColumns) ? 'true' : 'false' }}" data-sortable="true">Pedido</th>
          <th data-field="created_at" data-visible="{{ in_array('created_at', $visibleColumns) || empty($visibleColumns) ? 'true' : 'false' }}" data-sortable="true">Data</th>
          <th data-field="valor" data-visible="{{ in_array('valor', $visibleColumns) || empty($visibleColumns) ? 'true' : 'false' }}" data-sortable="true">Pagamento</th>
          ...
          <th data-field="mercado_pago_id" data-visible="{{ in_array('mercado_pago_id', $visibleColumns) || empty($visibleColumns) ? 'true' : 'false' }}" data-sortable="true">ID da Transação</th>
          <th data-field="cliente_nome" data-visible="{{ in_array('cliente_nome', $visibleColumns) || empty($visibleColumns) ? 'true' : 'false' }}" data-sortable="true">Cliente</th>
