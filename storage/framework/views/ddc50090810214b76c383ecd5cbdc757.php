<?php $__env->startSection('title', 'Loja de Plugins'); ?>

<?php $__env->startSection('content'); ?>
    <div class="container">
        <h4 class="py-3 mb-2">
            <span class="text-muted fw-light"><?php echo e(config('variables.templateName', 'TemplateName')); ?> / </span>Loja de Plugins
        </h4>
                        ...
                        <div class="col-md-12 pt-3 card plugin_card popular_card position-relative">
                            ...
                            <div class="row">
                                <div class="col-md-4 col-4">
                                    <img src="<?php echo e($plugin['image_url']); ?>" class="img-fluid rounded-start"
                                        style="border-radius: 10px;" width="95%" alt="<?php echo e($plugin['name']); ?>">
                                </div>
                                <div class="col-md-8 col-8">
                                    <h4 class=""><?php echo e($plugin['name']); ?></h4>
                                    <p>
                                        <i class="fa fa-money-bill"></i> R$ <?php echo e($plugin['price']); ?>

                                        <br>
                                        <span
                                            style="font-size: 13px;justify-content: normal;line-height: 15px;display: flex;margin-top: 10px;color: #828282!important;">
                                            <?php echo e(\Illuminate\Support\Str::limit($plugin['description'], 100)); ?>

                                        </span>
                                    </p>
                                    <?php if(!isset($activeModules[$pluginNameLower])): ?>
                                        <button class="d-block mt-2 mb-1 btn w-100 btn-primary"
                                            onclick="initiatePurchase(<?php echo e($plugin['id']); ?>)">Comprar</button>
                                    <?php else: ?>
                                ...
                                <div class="col-md-12 pb-4 col-12">
                                    <span>
                                        ...
                                        <span
                                            class="text-muted fw-light"><?php echo e(config('variables.templateName', 'TemplateName')); ?></span>
                                    </span>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        function initiatePurchase(pluginId) {
            fetch('<?php echo e(route('plugins.initiatePurchase')); ?>', {
                    method: 'POST',
                    headers: {
        ...
        function checkPaymentStatus() {
            ...
            fetch(`<?php echo e(route('plugins.checkPaymentStatus')); ?>`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
                ...
                .then(data => {
                    if (data.error) {
                        ...
                        if (data.status === 'confirmed' || data.status === 'approved') {
                            ...
                            initiatePluginInstallation(currentPaymentId, <?php echo e(auth()->user()->id); ?>);
                        }
                    }
                })
                .catch(error => {
        ...
        function initiatePluginInstallation(paymentId, userId) {
            ...
            const url = '<?php echo e(route('plugins.geraNew')); ?>';
...
<?php echo $__env->make('layouts/layoutMaster', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/gestor-pro.vipconnect.top/public_html/resources/views/plugins/index.blade.php ENDPATH**/ ?>